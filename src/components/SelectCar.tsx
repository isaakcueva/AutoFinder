...
const SearchView = () => {
  ...
  type RootStackParamList = {
    Home: { selectedCarIndex: number; car: { id: string; name: string; price: string; image: string } };
    SelectCar: { selectedCarIndex: number };
  };
  ...
  const selectedCarIndex = route.params?.selectedCarIndex || 0;
  interface CarData {
    id: string;
    name: string;
    price: string;
  ...
  const handleSelectCar = (car: CarData) => {
    console.log("Datos del carro seleccionado:", car);
    navigation.navigate('Home', {
      selectedCarIndex, // Índice del marco que se está actualizando
      car: {
        id: car.id,
        name: car.name,
        price: car.price,
  ...
  const renderItem = ({ item }: any) => (
    <View style={styles.card}>
      ...
      style={styles.button}
      onPress={() => {
        console.log('Auto seleccionado:', item); // <---- Revisa qué datos contiene `item`
        handleSelectCar({
          id: item.idcarro,
          name: item.nombrecarro,
          price: item.preciocarro,
          image: item.url,
        });
      }}
    >
...
const styles = StyleSheet.create({
  ...
  footerContainer: {
    ...
    bottom: 0,
    height: 60,
    zIndex: 10, // Forzar la posición en la parte superior del contenido
    shadowColor: 'transparent', // Elimina cualquier sombra que pueda generar transparencia
  },
  Container: {
    flex: 1,
    justifyContent: 'center',
